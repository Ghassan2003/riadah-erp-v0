...
import {
  ...
  Building2, Landmark, FileSignature, CreditCard, Calendar, Receipt, Wallet,
...
export default function DashboardPage() {
  ...
  /* ── KPI Cards ── */
  const kpiCards = [
    { title: t('totalOrders'), value: d.sales?.total_orders ?? 0, icon: ShoppingCart, color: 'blue', sub: `${d.sales?.confirmed_orders ?? 0} ${t('confirmedOrders')}`, path: '/orders' },
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          },
          {
            ...
            value: d.projects?.active_projects ?? 0,
            icon: FolderKanban,
            color: '#ef4444',
            sparkData: null,
            sparkColor: '#8b5cf6',
      ...
      {/* ── Charts Row 3: Payroll + Contract Status ── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Payroll Distribution Bar Chart */}
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-5 shadow-sm border border-gray-100 dark:border-gray-700">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">{t('totalSalaries')}</h3>
          <div className="h-52 sm:h-56">
            <ResponsiveContainer width="100%" height="100%">
              {(() => {
                const payData = d.payroll ? [
                  { name: t('paid'), value: d.payroll.total_paid_this_month || 0 },
                  { name: t('pending'), value: d.payroll.pending_payments || 0 },
                  { name: t('advances'), value: d.payroll.total_advances || 0 },
                ] : [];
                return payData.length > 0 ? (
                  <BarChart data={payData} margin={{ top: 5, right: 5, bottom: 5, left: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#374151' : '#f1f5f9'} />
                    <XAxis dataKey="name" tick={{ fontSize: 10, fill: isDark ? '#9CA3AF' : '#6B7280' }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fontSize: 10, fill: isDark ? '#9CA3AF' : '#6B7280' }} axisLine={false} tickLine={false} />
                    <Tooltip content={<ChartTooltip isDark={isDark} locale={locale} fmt={fmt} />} />
