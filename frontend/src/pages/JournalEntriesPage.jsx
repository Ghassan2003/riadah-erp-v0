/**
 * Journal Entries page - create, edit, manage, and export accounting journal entries.
 * Supports creating/editing entries with multiple debit/credit lines (double-entry).
 * Includes date range filter, account filter, Excel export, and delete draft entries.
 */

import { useState, useEffect } from 'react';
import { journalEntriesAPI, accountsAPI, exportAPI } from '../api';
import toast from 'react-hot-toast';
import { useI18n } from '../i18n/I18nContext';
import { useAuth } from '../context/AuthContext';
import {
  Search, Plus, X, FileText, Check, Ban, Eye, Edit2, Filter,
  AlertCircle, ArrowLeftRight, Trash2, Download,
} from 'lucide-react';

export default function JournalEntriesPage() {
  const { t, locale } = useI18n();
  const { isAccountant } = useAuth();
  const canManage = isAccountant;
  ...
  // Form state for creating / editing entry
  ...
  // ── Derived ────────────────────────────────────────────────────
  const totalDebit = transactions.filter(tx => tx.transaction_type === 'debit').reduce((sum, tx) => sum + (parseFloat(tx.amount) || 0), 0);
  const totalCredit = transactions.filter(tx => tx.transaction_type === 'credit').reduce((sum, tx) => sum + (parseFloat(tx.amount) || 0), 0);
  ...
  useEffect(() => {
    fetchEntries();
  }, [search, filterType, filterPosted, dateFrom, dateTo, filterAccount, page]);
  ...
  const openEdit = async (entryId) => {
    try {
      const res = await journalEntriesAPI.get(entryId);
      ...
      setEntryForm({
        ...
        entry_date: detail.entry_date || new Date().toISOString().split('T')[0],
      ...
      if ((detail.transactions || []).length < 2) {
        setTransactions(prev => [
          ...prev,
          { account: '', transaction_type: prev[0]?.transaction_type === 'debit' ? 'credit' : 'debit', amount: '', description: '' },
        ]);
      }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await journalEntriesAPI.update(editingEntryId, data);
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const res = await exportAPI.entries();
  ...
  // ── Transaction helpers (shared for create & edit) ─────────────
  ...
  const renderEntryForm = (onSubmit, submitLabel) => (
    <form onSubmit={onSubmit} className="p-5 space-y-4">
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="button" onClick={() => { setShowCreateModal(false); setShowEditModal(false); setEditingEntryId(null); }}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleExport} disabled={exporting}
            className="flex items-center gap-2 px-4 py-2.5 bg-green-600 dark:bg-green-700 text-white rounded-lg hover:bg-green-700 dark:hover:bg-green-800 transition-colors shadow-sm disabled:opacity-50">
            {exporting ? (
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        {/* Row 2: Date Range + Account Filter */}
        <div className="flex flex-col sm:flex-row gap-3 mt-3">
          <div className="flex-1 flex flex-col sm:flex-row gap-3">
            <div className="flex-1">
              ...
              <input type="date" value={dateFrom}
                ...
                className="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-transparent outline-none text-sm" />
            </div>
            <div className="flex-1">
          ...
          <div className="flex-1">
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{t('filterByAccount')}</label>
            <select value={filterAccount} onChange={(e) => { setFilterAccount(e.target.value); setPage(1); }}
              className="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-transparent outline-none bg-white dark:bg-gray-700 text-sm">
                  ...
                  <tr key={entry.id} className="border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {/* View */}
                        <button onClick={() => openDetail(entry)} title={t('view')}
                        ...
                        {canManage && !entry.is_posted && (
                          <button onClick={() => openEdit(entry.id)} title={t('edit')}
                            className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 p-1.5 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">
                            <Edit2 className="w-4 h-4" />
                          </button>
                        )}
                        {/* Post (draft only) */}
                        {canManage && !entry.is_posted && (
                          <button onClick={() => handlePost(entry.id)} disabled={postingId === entry.id} title={t('postEntry')}
                        ...
                        {/* Reverse (posted only) */}
                        {canManage && entry.is_posted && (
                          <button onClick={() => handleReverse(entry.id)} title={t('reverseEntry')}
                        ...
                        {/* Delete (draft only) */}
                        {canManage && !entry.is_posted && (
                          <button onClick={() => handleDelete(entry.id)} disabled={deletingId === entry.id} title={t('delete')}
                            className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors disabled:opacity-50">
                            {deletingId === entry.id ? (
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b dark:border-gray-700">
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">{t('addEntry')}</h3>
            ...
            {renderEntryForm(handleCreate, t('create'))}
          </div>
        </div>
      )}
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b dark:border-gray-700">
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">{t('editEntry')}</h3>
              <button onClick={() => { setShowEditModal(false); setEditingEntryId(null); }} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 p-1">
                <X className="w-5 h-5" />
              </button>
            </div>
            {renderEntryForm(handleUpdate, t('save'))}
          </div>
        </div>
      )}
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-2">
                {canManage && !selectedEntry.is_posted && (
                  <button onClick={() => handlePost(selectedEntry.id)} disabled={postingId === selectedEntry.id}
                    className="flex items-center gap-2 px-4 py-2 bg-green-600 dark:bg-green-700 text-white rounded-lg hover:bg-green-700 dark:hover:bg-green-800 transition-colors text-sm font-medium disabled:opacity-50">
