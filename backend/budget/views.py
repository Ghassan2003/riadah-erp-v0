...
class BudgetDetailView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        budget = serializer.save()
        # Recalculate remaining_amount if total_budget changed
        if 'total_budget' in serializer.validated_data:
            budget.remaining_amount = budget.total_budget - budget.utilized_amount
            budget.save(update_fields=['remaining_amount'])
        return Response({
...
class BudgetCategoryDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        category = serializer.save()
        # Recalculate remaining_amount if allocated_amount changed
        if 'allocated_amount' in serializer.validated_data:
            category.remaining_amount = category.allocated_amount - category.utilized_amount
            category.save(update_fields=['remaining_amount'])
        return Response({
