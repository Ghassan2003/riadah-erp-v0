...
class TestSalesOrderModel:
    ...
    def test_calculate_total(self, admin_user, customer):
        """اختبار حساب الإجمالي."""
        from sales.models import SalesOrder, SalesOrderItem
        order = SalesOrder.objects.create(customer=customer, created_by=admin_user)
        SalesOrderItem.objects.create(
            order=order,
            product_name='منتج أول',
            quantity=5,
            unit_price=50.00,
        )
        SalesOrderItem.objects.create(
            order=order,
            product_name='منتج ثاني',
            quantity=3,
            unit_price=30.00,
        )
    ...
    @pytest.mark.skip(reason="inventory module removed - confirm_order no longer checks inventory")
    ...
    def test_confirm_non_draft_order(self, admin_user, customer):
        """اختبار تأكيد أمر ليس في حالة مسودة."""
        from sales.models import SalesOrder, SalesOrderItem
        order = SalesOrder.objects.create(customer=customer, created_by=admin_user)
        SalesOrderItem.objects.create(
            order=order, product_name='منتج اختبار', quantity=5, unit_price=50.00,
        )
        order.confirm_order()
    ...
    @pytest.mark.skip(reason="inventory module removed - cancel_order no longer restores inventory")
    ...
    def test_change_status_valid_transition(self, admin_user, customer):
...
class TestSalesOrderItemModel:
    ...
    def test_auto_calculate_subtotal(self, admin_user, customer):
    ...
    def test_unique_order_product(self, admin_user, customer):
        ...
        with pytest.raises(Exception):
            SalesOrderItem.objects.create(
                order=order, product_name='منتج اختبار', quantity=3, unit_price=50.00,
            )
