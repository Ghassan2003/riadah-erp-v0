...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    # Note: Email existence check is intentionally NOT done here
    # to prevent email enumeration attacks. The view handles
    # non-existent emails gracefully by returning a generic success message.
...
class ResetPasswordSerializer(serializers.Serializer):
    ...
    # Note: Email existence check is intentionally NOT done here
    # to prevent email enumeration attacks. The view handles
    # non-existent emails gracefully.
...
class InvitationCreateSerializer(serializers.Serializer):
    """Serializer for creating employee invitations (admin/HR only)."""
    email = serializers.EmailField()
    role = serializers.ChoiceField(choices=User.ROLE_CHOICES)
    ...
    def validate_email(self, value):
        ...
        if User.objects.filter(email__iexact=email).exists():
            raise serializers.ValidationError('يوجد مستخدم مسجل بهذا البريد الإلكتروني بالفعل')
        # Check if invitation already pending
        if Invitation.objects.filter(email__iexact=email, status='pending').exists():
            raise serializers.ValidationError('يوجد دعوة معلقة لهذا البريد الإلكتروني بالفعل')
        return email

    def validate_role(self, value):
        allowed_roles = ['admin', 'warehouse', 'sales', 'accountant', 'hr', 'purchasing', 'project_manager']
        if value not in allowed_roles:
            raise serializers.ValidationError('الدور المحدد غير صالح')
        return value
...
class InvitationAcceptSerializer(serializers.Serializer):
    ...
    def validate_token(self, value):
        try:
            invitation = Invitation.objects.select_related('invited_by').get(token=value)
        except Invitation.DoesNotExist:
            raise serializers.ValidationError('رمز الدعوة غير صالح')

        if invitation.status != 'pending':
            raise serializers.ValidationError('هذه الدعوة غير صالحة أو تم استخدامها بالفعل')
        if invitation.is_expired:
            invitation.status = 'expired'
            invitation.save(update_fields=['status'])
            raise serializers.ValidationError('انتهت صلاحية هذه الدعوة')
    ...
    def validate(self, attrs):
        ...
        MinLengthValidator(8).validate(attrs['password'], user)
        ComplexityValidator().validate(attrs['password'], user)
        UsernameSimilarityArabicValidator().validate(attrs['password'], user)
        SequentialCharsValidator().validate(attrs['password'], user)
...
class InvitationListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Invitation
        fields = (
            'id', 'email', 'role', 'role_display', 'first_name', 'last_name',
            'invited_by_name', 'invited_by_username',
            'status', 'status_display', 'created_at', 'expires_at', 'accepted_at',
        )
