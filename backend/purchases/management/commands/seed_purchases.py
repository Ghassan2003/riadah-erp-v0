...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not admin_user:
            self.stdout.write(self.style.WARNING('No admin user found, skipping purchase orders.'))
            return
        ...
        orders_data = [
            {
                ...
                'items': [
                    {'product_name': 'حاسوب محمول Dell Latitude', 'quantity': 10, 'unit_price': 4200.00},
                    {'product_name': 'شاشة عرض 27 بوصة', 'quantity': 5, 'unit_price': 1100.00},
                ],
            },
            {
                'supplier': suppliers[1 % len(suppliers)],
                'status': 'received',
                'expected_date': date.today() - timedelta(days=5),
                'notes': 'طلب أثاث لمكتب الإدارة',
                'items': [
                    {'product_name': 'مكتب خشبي فاخر', 'quantity': 3, 'unit_price': 1700.00},
                ],
            },
            {
                ...
                'items': [
                    {'product_name': 'ورق طباعة A4 (رزمة)', 'quantity': 50, 'unit_price': 40.00},
                    {'product_name': 'حبر طابعة HP LaserJet', 'quantity': 20, 'unit_price': 230.00},
                ],
            },
            {
                'supplier': suppliers[3 % len(suppliers)],
                'status': 'partial',
                'expected_date': date.today() + timedelta(days=3),
                'notes': 'طلب حبر ولوازم طباعة',
                'items': [
                    {'product_name': 'حبر طابعة Canon', 'quantity': 30, 'unit_price': 240.00},
                ],
            },
        ]
