...
class PurchaseRequisitionListSerializer(serializers.ModelSerializer):
    """مسلسل خفيف لعرض قائمة طلبات الشراء."""
    ...
    class Meta:
        ...
        fields = (
            'id', 'requisition_number', 'title', 'priority', 'priority_display',
            'status', 'status_display', 'requested_by', 'requested_by_name',
            'department', 'department_name', 'required_date', 'estimated_budget',
            'items_count', 'total_value', 'created_at',
        )
        read_only_fields = ('id', 'requisition_number', 'created_at')
    ...
    def get_total_value(self, obj):
        total = obj.items.aggregate(
            total=Coalesce(Sum('subtotal'), Value(0), output_field=DecimalField(max_digits=14, decimal_places=2))
        ).get('total')
        return total


class PurchaseRequisitionCreateSerializer(serializers.ModelSerializer):
    """مسلسل لإنشاء طلب شراء مع البندود."""
    ...
    class Meta:
        model = PurchaseRequisition
        fields = (
            'title', 'description', 'priority', 'required_date',
            'department', 'estimated_budget', 'notes', 'items',
        )

    def validate_items(self, value):
        if not value:
            raise serializers.ValidationError('يجب إضافة منتج واحد على الأقل')
        return value
...
class PurchaseRequisitionDetailSerializer(serializers.ModelSerializer):
    """مسلسل تفصيلي لعرض طلب شراء مع البندود."""
    ...
    class Meta:
        model = PurchaseRequisition
        fields = (
            'id', 'requisition_number', 'title', 'description', 'priority',
            'priority_display', 'status', 'status_display', 'requested_by',
            'requested_by_name', 'approved_by', 'approved_by_name',
            'approved_at', 'rejection_reason', 'required_date',
            'department', 'department_name', 'estimated_budget',
            'notes', 'items', 'total_value', 'created_at', 'updated_at',
        )
        read_only_fields = ('id', 'requisition_number', 'created_at', 'updated_at')
...
class PurchaseRequisitionApproveSerializer(serializers.Serializer):
    """مسلسل للموافقة أو الرفض على طلب الشراء."""

    action = serializers.ChoiceField(
        choices=['approve', 'reject'],
        error_messages={'invalid_choice': 'الإجراء يجب أن يكون "approve" أو "reject"'},
    )
