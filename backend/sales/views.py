...
class CustomerListView(PermissionRequiredMixin, generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if show_deleted and show_deleted.lower() == 'true':
            base = Customer.all_objects.all() if hasattr(Customer, 'all_objects') else Customer.objects.all()
        else:
            base = Customer.objects.filter(is_active=True)
        return base.annotate(
            orders_count=Count('orders', filter=Q(orders__status__in=['draft', 'confirmed', 'shipped', 'delivered']))
        )
    ...
    def get_queryset(self):
        return Customer.objects.annotate(
            ...
            total_spent=Coalesce(
                Sum('orders__total_amount', filter=Q(orders__status__in=['confirmed', 'shipped', 'delivered'])),
...
class SalesOrderListView(PermissionRequiredMixin, generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = SalesOrder.objects.select_related('customer', 'created_by').annotate(
...
class SalesOrderDetailView(PermissionRequiredMixin, generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        return SalesOrder.objects.select_related('customer', 'created_by').prefetch_related('items')
