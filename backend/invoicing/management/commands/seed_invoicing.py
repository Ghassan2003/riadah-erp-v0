...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not customers:
            self.stdout.write('No customers found. Run seed_sales first.')
            return

        # Hardcoded product names and prices (inventory module removed)
        sample_products = [
            {'name': 'حاسوب محمول Dell Latitude', 'unit_price': 4200.00},
            {'name': 'شاشة عرض 27 بوصة Samsung', 'unit_price': 1100.00},
            {'name': 'طابعة ليزر HP LaserJet', 'unit_price': 2300.00},
        ]
