...
class MaintenanceScheduleCreateSerializer(serializers.ModelSerializer):
    """Serializer for creating a maintenance schedule."""

    class Meta:
        model = MaintenanceSchedule
        fields = (
            'equipment', 'maintenance_type', 'frequency_type', 'frequency_value',
            'last_performed', 'next_due', 'assigned_to', 'estimated_cost',
            'estimated_hours', 'priority', 'is_active', 'notes',
        )

    def validate(self, attrs):
        if not attrs.get('next_due') and attrs.get('last_performed') and attrs.get('frequency_type') and attrs.get('frequency_value'):
            # Auto-calculate next_due if not provided
            ...
            if attrs['frequency_type'] == 'custom':
                attrs['next_due'] = attrs['last_performed'] + timedelta(days=attrs['frequency_value'])
            else:
                days = freq_map[attrs['frequency_type']]
                attrs['next_due'] = attrs['last_performed'] + timedelta(days=days * attrs['frequency_value'])
        return attrs
