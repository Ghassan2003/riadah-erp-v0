...
class FinancialTransactionCreateView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Update account balances based on transaction type (only for completed transactions)
        account = trx.account
...
class ReconciliationUpdateView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        diff = instance.actual_balance - instance.system_balance
        instance.status = 'reconciled' if diff == 0 else 'discrepancy'
        instance.save()
        return Response({
